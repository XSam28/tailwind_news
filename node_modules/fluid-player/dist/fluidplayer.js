var fluidPlayerScriptLocation = function () {
    ...
    if (currentSrc) {
        currentSrc = currentSrc.substring(0, currentSrc.lastIndexOf('/') + 1);
...
var fluidPlayerClass = {
    ...
    subtitlesParseScript: '/scripts/vtt.min.js',
    instances: [],
    ...
    adKeytimePlay: function (keyTime) {
        ...
        if (!player.timerPool[keyTime]) {
            // проверяем, есть ли реклама в предыдущем видео, которое пользователь мог перемотать
            var tempAdPoints = Object.keys(player.timerPool);
            var r = 0;
            for (var i = 0; i < tempAdPoints.length; i++) {
                var p = parseInt(tempAdPoints[i]);
                if ((p < keyTime) && (p > r)) {
                    r = p;
                }
            }
